...
cfg.localConfig = {
    ...
    mineLoop = false,
    mineLoopOffset = { x = 0, y = 0, z = 64 },
    mineLoopCommand = "branch 64 32"
}
...
local WIRELESS_MODEM_SLOT = "wireless_modem_slot"
...
local PLACEABLE_BLOCKS = {
    ["minecraft:cobblestone"] = true,
    ["minecraft:cobbled_deepslate"] = true,
    ["minecraft:netherrack"] = true,
}

local REDNET_PROTOCOL = "ccmine"
...
local wrapt = (function()
    ...
    -- Add a method to set the global origin
    public.setOrigin = function(x, y, z, direction)
        self.gX = x
        self.gY = y
        self.gZ = z
    ...
    -- Add getters for global coordinates
    public.getGlobalX = function() return self.gX + self.x end
    public.getGlobalY = function() return self.gY + self.y end
    public.getGlobalZ = function() return self.gZ + self.z end
...
local slots = (function()
    ...
    local generateDescription = function(config)
        local desc = {
            ...
            [WIRELESS_MODEM_SLOT] = "Wireless Modem",
        }
    ...
    public.assignSlots = function(config)
        ...
        if config.useWirelessModem then
            slotAssignments[WIRELESS_MODEM_SLOT] = currentSlot
            currentSlot = currentSlot + 1
        end
...
ui.promptYesNo = function(question)
    helper.print(question .. " (y/n): ")
    while true do
        local input = helper.read()
        if input == "y" then
            return true
        elseif input == "n" then
            ...
            helper.print("Invalid input, please type 'y' or 'n'")
        end
    end
end

ui.promptNumber = function(question)
    ...
    while true do
        ...
        if number then
            ...
            helper.print("Invalid input, please type a number")
        end
    end
end
...
local network = {
    send = function(message)
        if (not useNetwork) then
            return
        end

        local label = os.getComputerLabel() or ("Turtle_" .. os.getComputerID())

        -- Use the global getters to compute current absolute position
        local globalX = wrapt.getGlobalX()
        local globalY = wrapt.getGlobalY()
        local globalZ = wrapt.getGlobalZ()

        local formattedMessage = string.format("[%s] [%d,%d,%d] %s",
            label,
            globalX,
            globalY,
            globalZ,
        ...
        execWithSlot(turtle.equipLeft, WIRELESS_MODEM_SLOT)
        peripheral.find("modem", rednet.open)
        if rednet.isOpen() then
            rednet.broadcast(formattedMessage, REDNET_PROTOCOL)
        end
        rednet.close()
        execWithSlot(turtle.equipLeft, WIRELESS_MODEM_SLOT)
    end,
...
local function isOre(inspectFunction)
    local success, data = inspectFunction()
    if success and (helper.stringEndsWith(data.name, "_ore") or helper.stringEndsWith(data.name, "_debris")) then
        return true
    end
    return false
end
...
local function sortInventory(sortFuel)
    ...
    if initCobbleData and not PLACEABLE_BLOCKS[initCobbleData.name] then
    ...
    for i = slots.get(MISC_SLOT), 16 do
        ...
        if curData then
            if PLACEABLE_BLOCKS[curData.name] then
                wrapt.select(i)
...
local function dropOffEntangledChest(diggingArea)
    ...
    local delta = {
        x = selectedOption.x - curPosition.x,
        y = selectedOption.y - curPosition.y,
        z = selectedOption.z -
            curPosition.z
    }
    if delta.y < 0 then
...
local function executeDigging(layers, diggingArea, chestData, config)
    local blocksCount = helper.tableLength(diggingArea)
    local counter = 0
    for layerIndex, layer in ipairs(layers) do
        for blockIndex, block in ipairs(layer) do
            ...
            -- Only send if progress has moved by at least 5%
            local currentPercentage = math.floor((counter / blocksCount) * 100)
            if currentPercentage % 5 == 0 and currentPercentage ~= lastSentPercentage then
                network.send("Working: " .. counter .. "/" .. blocksCount .. " (" .. currentPercentage .. "%)")
...
local function getValidatedRegion(config, default)
    ...
    while true do
        ...
        if validationResult == SUCCESS then
            return genRegion, shape
        end
        ui.showValidationError(validationResult)
    end
end

local function getLayerSeparationAxis(command)
    ...
    if command == "branch" then
        helper.print("Using Z layer separation")
        return "z"
    end
    return "y"
end
...
local function launchDigging(config, default)
    local diggingArea, shape = getValidatedRegion(config, default)
    local layers = region.createLayersFromArea(diggingArea, getLayerSeparationAxis(shape.command))
...
local function digToLevel(currentLevel, targetLevel)
    local totalYDiff = math.abs(currentLevel - targetLevel)
    local isAbove = currentLevel > targetLevel
    local isBelow = currentLevel < targetLevel

    for i = 1, totalYDiff do
        if isAbove then
            repeat wrapt.digDown() until wrapt.down(true)
        elseif isBelow then
            repeat wrapt.digUp() until wrapt.up(true)
        end
    end
end
...
local function main()
    ...
    useNetwork = config.useWirelessModem
    network.send("Ping")

    local mineLoop
    local default
    local currentLevelArg
    local targetLevelArg
    if config.mineLoop then
        helper.print("Mine loop command: " .. config.mineLoopCommand)
        mineLoop = ui.promptYesNo("Use mine loop?")
    elseif config.defaultCommand then
        helper.print("Default command: " .. config.defaultCommand)
        default = ui.promptYesNo("Use default command?")
    end
    local useSetLevel = ui.promptYesNo("Set target level?")
    if useSetLevel then
        currentLevelArg = ui.promptNumber("Current level")
        targetLevelArg = ui.promptNumber("Target level")
    end

    -- Ask for starting position and direction
    helper.print("Enter current coords and facing direction (X Y Z [N/S/E/W]):")
    local input = helper.read()
    local posInput = helper.splitString(input) -- splits by space

    -- We expect 4 items: X, Y, Z, and Direction
    if #posInput == 4 then
        local x, y, z = tonumber(posInput[1]), tonumber(posInput[2]), tonumber(posInput[3])
        local dirChar = string.upper(posInput[4])

        -- Logic to convert char to internal 0-3
        local dirMap = { ["S"] = 0, ["W"] = 1, ["N"] = 2, ["E"] = 3 }
        local dir = dirMap[dirChar] or 0

        wrapt.setOrigin(x, y, z, dir)
        helper.print("State initialized at " .. x .. "," .. y .. "," .. z .. " facing " .. dirChar)
    else
        helper.printWarning("Invalid input format. Expected: X Y Z [N/S/E/W]. Defaulting to 0,0,0 facing South.")
        wrapt.setPosition(0, 0, 0)
        wrapt.setDirection(0)
    end

    network.send("Mining started")

    if currentLevelArg and targetLevelArg then
        digToLevel(tonumber(currentLevelArg), tonumber(targetLevelArg))
    end

    if mineLoop then
        launchMineLoop(config, true)
    else
        launchDigging(config, default)
    end

    network.send("Mining complete")
    helper.print("Mining complete")
end
